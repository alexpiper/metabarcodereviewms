...
library(knitr)
knitr::opts_chunk$set(echo = TRUE, eval=FALSE, fig.show = "asis", fig.keep = "all")
...
sapply(c("rentrez","bold", "seqinr","taxize","biofiles", "tidyverse",
         "scales","gridExtra","grid","readr","fulltext","data.table",
...
### Keyword processing of retrieved articles

The keywords used are:

Figure 1A - Invasive species keywords
  + invasive
  + detection
  + surveillance
  + diagnostic
...
Figure 1B - Sequencing platform keywords
  + 454 OR pyroseq
  + hiseq
  + miseq
  + nextseq
  + novaseq
  + pacbio OR pacific biosciences
  + mgiseq
  + ion torrent
  + nanopore OR minion OR promethion
...
* Figure 1a - Published for for all metabarcoding studies, and those containing keywords in title or abstract relevant to invasive insect surveillance
* Figure 1b - Sequencing platforms used in the above metabarcoding studies displayed as a proportion for each year.
...
```{r Figure 1, eval=TRUE, message=FALSE,warning=FALSE}
#Read in data
df_1a <- read_csv("data/fig1/litsearch_1a.csv") %>%
...
Figure 2 is an overview of common metabarcoding workflows revealed by a literature search. The figure itself was made entirely in Adobe Illustrator.
...
```{R plot figure 3a, eval=TRUE,warnings=FALSE,message=FALSE,errors=FALSE}
#Read in data
...
```{r Figure 3b , eval=TRUE, warning=FALSE,message=FALSE}
...
The two sub figures were combined, then further edited in Adobe Illustrator

```{r plot combined figure}
Fig3 <- ggarrange(p3, p4, 
          ncol = 1, nrow = 2, labels = c("A","B"),
          heights = c(7, 4), widths= c(2,1))
```


## Figure 4 - Unique dual indexing overcomes issues of cross-contamination due to index switching

Figure 4 is an overview of common metabarcoding workflows revealed by a literature search. The figure itself was made entirely in Adobe Illustrator.
